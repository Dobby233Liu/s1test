...
EntryPoint:
		...
		movem.w (a0)+,d2/d5				; first VDP register value ($8004), VDP register increment/value for Z80 stop and reset release ($100)
		moveq	#SetupVDP_end-SetupVDP-1,d1		; VDP registers loop counter
   ...
   .wait_dma:
		move.w	(a6),ccr				; copy status register to CCR, resetting the 2nd write flag and setting the overflow flag if a DMA is in progress
   ...
   .load_z80:
		...
		move.w	#($2000-(Z80_Startup_end-Z80_Startup))-1,d1 ; size of remaining Z80 ram
   .clear_Z80_ram:
   ...
   .psg_loop:
	...
	.set_vdp_buffer:
		...
		move.w	d2,z80_port_1_control-z80_bus_request(a3) ; initialise port 1
		move.w	d2,z80_port_2_control-z80_bus_request(a3) ; initialise port 2
		move.w	d2,z80_expansion_control-z80_bus_request(a3) ; initialise port e
...
SetupValues:
   ...
   SetupVDP:
		...
		dc.b	($8500+(vram_sprites>>9))&$FF		; $857C; sprite attribute table starts at $F800
...
Tit_EnterCheat:
		...
		lea	(f_debugcheat).w,a1
		lea	(f_slomocheat).w,a2
...
Level_LoadObj:
	...
	Level_SkipClr:
        ...
        tst.b   (v_shield).w                ; is Sonic meant to have a shield?
        beq.s   Level_NoShield              ; if not, skip
        move.b  #id_ShieldItem,(v_objspace+$180).w  ; set to reload the shield object
...
SignpostArtLoad:
		...
		bra.w	NewPLC		; load signpost	patternsf
...
End_LoadSonic:
		...
		; move.b	#id_HUD,(v_objspace+$40).w ; don't load HUD object (todo 5)
		jsr	(ObjPosLoad).l
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		moveq	#0,d0
...
loc_DA10:
		bsr.w	loc_DA3C
		beq.s	loc_DA02
		tst.b	$04(a0)		; was this object in remember state?
